...
class DRL_MC_Controller:

    def __init__(self, model, control_frequency, N):

        self._neural_model = model
        self._control_frequency = control_frequency
        self._dt = 1 / control_frequency
        self._N = N  # Prediction horizon
    ...
    def act(self, target_states, state): 

        obs = self._compute_agent_vect(target_states, state)
...
if __name__ == "__main__":
    ...
    lstm_model_path = "/home/heracles-d2/training_envs/models/ppo_analytic_model_N_20_2025_12_09.zip"
    ...
    DRL = DRL_MC_Controller(PPO.load(lstm_model_path), control_frequency, horizon)
    ...
    for traj in trajs:
        ...
        while t < traj_duration:
            ...
            for i in range(horizon):
                ...
                target_x.append(x_ref)
                target_y.append(y_ref)

            target = np.array([x_ref, y_ref, yaw_ref, v_ref, w_ref])
            # DRL.set_current_velocities([lin_vel, ang_vel])
            # DRL.set_current_pose([pose_x, pose_y, pose_yaw])
            # DRL.set_target_pose([x_ref, y_ref, yaw_ref])
            # DRL.set_target_velocities([v_ref, w_ref])
            # DRL.set_gamma(gamma_cmd)
            x_init = np.array([pose_x, pose_y, pose_yaw, lin_vel, gamma])
            u = DRL.act(target_vector, x_init)

            gamma_list = []

            for i in range(horizon):

                v_ref, gamma_cmd = u[2*i], u[2*i+1]
                U_k = {'vf_ref': v_ref, 'gamma_cmd': gamma_cmd}
                gamma_list.append(gamma_cmd)
