...
class EPSF_Controller:
    ...
    def update_Jb_from_solution(self, x_k, X_opt, cost, u):

        epsilon = 0.1

        e0 = x_k[0:5] - self._target_vector[:, 0]
        ell_k = float(e0.T @ self._Q_t @ e0) + float((u[0:2]).T @ self._diff_R @ u[0:2])
    ...
    def build_mpc_classic_pb(self, x_init):
        ...
        for k in range(self._N):
            ...
            J_cost += ca.mtimes([(U[:, k]).T, ca.MX(self._diff_R), U[:, k]])
    ...
    def get_neural_prediction_states(self, action, x_init) :
        ...
        x_states = []
        x_states.append(x_init.tolist())
        for i in range(self._N):
            v_cmd, gamma_action = action[2*i], action[2*i + 1]
            x_next = self.rk4_step(x_init, action)
            x_next[4] =  ca.fmod(x_next[4] + ca.pi, 2 * ca.pi) - ca.pi
            x_states.append([float(x_next[0]), float(x_next[1]), float(x_next[2]), float(x_next[3]), float(x_next[4]), float(x_next[5]), float(x_next[6])])
            x_init = x_next
    ...
    def build_nlp(self, x_init, x_ref, u_ref, h):
        ...
        for k in range(h):
            ...
            cost += weight * state_error.T @ self._Q @ state_error
            cost += (1 - weight) * diff_cmd.T @ self._R @ diff_cmd
    ...
    def solve_nlp(self, nlp, sizes, u_ref, x_init, x_ref) : 
        ...
        if not stats['success']:
            ...
            for k in range(horizon):
                ...
                J_cost += float((U_opt[:, k]).T @ self._diff_R @ U_opt[:, k])
    ...
    def act(self, x_init) :
        ...
        return opt_cmd, X_opt, u_rl, U_opt_list, cost_red, need_init