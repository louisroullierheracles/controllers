...
class PSF_MC_Controller:
    ...
    def act(self, x_init) :
        ...
        target = self._target_vector[:, 0]
        self.set_target_pose(target[0:3])
        self.set_target_velocities(target[3:5])
        ...
        for i in range(self._N):
            ...
            u_cmd = self.get_neural_prediction(obs)[0]
            U_init.append(u_cmd)
            x_next = self.rk4_step(x_interm, u_cmd)
            x_next = np.array(x_next).squeeze()
            yaw = (x_next[2] + np.pi) % (2 * np.pi) - np.pi
            self._pose = [x_next[0], x_next[1], yaw]
            self._velocities = [self.get_velocities_from_cmd(u_cmd)[0], self.get_velocities_from_cmd(u_cmd)[1]]
            self.set_gamma(u_cmd[1])

            target = self._target_vector[:, i]
            self.set_target_pose(target[0:3])
            self.set_target_velocities(target[3:5])

            x_interm = x_next
            X_drl.append(x_interm)
            obs = self._compute_agent_vect()


        X_opt, U_opt = self.solve_global_problem(x_init, U_init)
        # v_ref_mean = np.mean(U_opt[0, :])
        # gamma_cmd_mean = np.mean(U_opt[1, :])
        # opt_cmd_mean= [v_ref_mean, gamma_cmd_mean]

        opt_cmd = U_opt[:, 0].tolist()

        return opt_cmd, U_init, U_opt, X_opt, X_drl
...
if __name__ == "__main__":
    ...
    bounds = {
            ...
            "x_min" : [31.0, 535.2, -np.pi], #xmin, ymin, thetamin, gammamin
            "x_max" : [75.6, 680.2, np.pi]  #xmax, ymax, thetamax, gammamax
    }

    R = np.diag([10.0, 1.0])  # Control cost
    ...
    target_vector = np.zeros((5, horizon + 1))

    with open("psf_log_mc.csv", "w", newline="") as f:

        writer = csv.writer(f)
        header = ["step", "x", "y", "theta"]
        for i in range(horizon):
            #write each element of X_opt "
            header += [f"v_ref_{i}", f"gamma_cmd_{i}"]
        ...
        for traj in trajs:
            ...
            while t < traj_duration + 15.0:
                ...
                for i in range(horizon):
                    ...
                    target_vector[:, i] = np.array([x_ref, y_ref, yaw_ref, v_ref, w_ref])

                PSF.set_target_vector(target_vector)

                x_init = np.array([pose_x, pose_y, pose_yaw])

                u, U_init, U_opt, X_opt, X_drl = PSF.act(x_init)
                v_ref, gamma_cmd = u

                u_psf.append([v_ref, gamma_cmd])
                u_drl.append(U_init[0])

                # if not np.allclose(U_opt, np.array(U_init).T, atol=1e-3):
                #     black_states_index.append(k)
                ...
                row = [k, pose_x, pose_y, pose_yaw]


                for n in range(horizon):
                    if X_opt is not None:
                        row += [U_init[n][0], U_init[n][1]]
                        row += [X_opt[0, n], X_opt[1, n], X_opt[2, n]] 
                row += [1 if k in black_states_index else 0]
                ...
                new_obstacle = np.array([[pose_x + 1.0, pose_y + 10.0],
                                        [pose_x + 1.0, pose_y + 8.0],
                                        [pose_x - 1.0, pose_y + 8.0],
                                        [pose_x - 1.0, pose_y + 10.0]])

                if random.random() < 0.05:
                    obstacles.append(new_obstacle)
                    PSF.set_obstacles(obstacles)
    ...
    for obs in obstacles:
        ...
        ellipse = PSF.obstacle_to_ellipse(obs, safety_radius=1.0)
